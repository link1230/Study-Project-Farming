...
var corn_plant_sence = preload("res://sences/object/corn/corn_water.tscn")
var tomato_plant_sence = preload("res://sences/object/tomato/tomato_water.tscn")
...
func _unhandled_input(event: InputEvent) -> void:
	if event.is_action_pressed("remove_dirt"):
		if ToolsManager.selected_tool == DataTypes.Tools.TillGround:
			get_cell_under_mouse()
			remove_crop()
			
	elif event.is_action_pressed("cut"):
		if ToolsManager.selected_tool == DataTypes.Tools.PlantCorn or ToolsManager.selected_tool == DataTypes.Tools.PlantTomato  :
			get_cell_under_mouse()
			add_crop()
...
func add_crop()->void:
	if distance < 20.0:
		if ToolsManager.selected_tool == DataTypes.Tools.PlantCorn:
			var corn_instance = corn_plant_sence.instantiate() as Node2D
			corn_instance.global_position = local_cell_position
			get_parent().find_child("CropFields").add_child(corn_instance)
			
		if ToolsManager.selected_tool == DataTypes.Tools.PlantTomato:
			var tomato_instance = tomato_plant_sence.instantiate() as Node2D
